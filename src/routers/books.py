...
def tokenize_to_html(text: str) -> str:
    ...
    escape_text = lambda s: html.escape(s, quote=True) # для атрибутов (например, id)
    parts = []
    for sentence in SENTENCE_SPLIT_RE.split(text):
        ...
        for token in TOKENIZE_RE.findall(sentence):
            if token.isalnum() or '-' in token:
                esc_text = escape_text(token)  # безопасен в id="..." # безопасен в >...<
                parts.append(f' <span id="{esc_text}" class="word">{esc_text}</span>')
            else:
                parts.append(token)
        parts.append("</span> ")
    return ''.join(parts)
